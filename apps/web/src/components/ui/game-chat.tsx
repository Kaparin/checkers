...
export function GameChat({ gameId }: { gameId: string }) {
  ...
  if (!isOpen) {
    return (
      <button
        ...
        className="fixed bottom-4 right-4 z-30 w-12 h-12 bg-accent text-white rounded-full shadow-lg flex items-center justify-center hover:bg-accent-hover transition-colors"
      >
  ...
  return (
    <div className="fixed bottom-4 right-4 z-30 w-72 h-80 bg-bg-card border border-border rounded-2xl shadow-2xl flex flex-col overflow-hidden">
      {/* Header */}
